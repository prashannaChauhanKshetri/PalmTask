"""OpenAI API client wrapper for embedding generation and chat completions."""
...
class OpenAIClient:
    """Async client wrapper for OpenAI API operations."""
    ...
    async def get_embeddings(self, texts: list[str]) -> list[list[float]]:
        """Generate vector embeddings for a list of text strings using text-embedding-3-small.

        Args:
            texts: List of text chunks to embed.

        Returns:
            List of 1536-dimensional float embedding vectors.
        """
        if not texts:
            return []
        ...
        logger.info(
            ...
            extra={"count": len(texts), "model": self.embedding_model},
        )
